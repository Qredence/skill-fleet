...
class GatherRequirementsSignature(dspy.Signature):
    ...
    # INPUT FIELDS
    task_description: str = dspy.InputField(desc="User's task description for skill creation")
    ...
    domain: Literal[
        ...
        "domain",
        "memory",
    ] = dspy.OutputField(desc="Primary domain category for this skill")
    ...
    topics: list[str] = dspy.OutputField(desc="3-7 specific topics this skill should cover")

    parent_skills: list[str] = dspy.OutputField(
        desc="Prerequisite skills user should know (e.g., ['python/basics', 'testing/pytest'])"
    )

    related_skills: list[str] = dspy.OutputField(desc="Related skills for cross-referencing")
...
class GenerateSkillContentSignature(dspy.Signature):
    ...
    content_plan: str = dspy.InputField(desc="Content structure and key points to cover")
...
if __name__ == "__main__":
    ...
    result = gather(task_description="Create a skill for async Python programming with asyncio")
