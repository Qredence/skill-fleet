    ...
    def _group_by_task_type(self) -> dict[str, Any]:
        ...
        for execution in self.executions:
            groups[execution.task_type].append({
                "count": len([e for e in self.executions if e.task_type == execution.task_type]),
                "avg_time": sum(e.execution_time_seconds for e in self.executions if e.task_type == execution.task_type) / len([e for e in self.executions if e.task_type == execution.task_type]),
                "total_time": sum(e.execution_time_seconds for e in self.executions if e.task_type == execution.task_type),
                "success_rate": sum(1 for e in self.executions if e.task_type == execution.task_type and e.success) / len([e for e in self.executions if e.task_type == execution.task_type]) if self.executions else 0,
            })
        return dict(groups)
    ...
    def _group_by_optimizer_type(self) -> dict[str, Any]:
        ...
        for opt in self.optimizations:
            groups[opt.optimizer_type].append({
                "count": len([o for o in self.optimizations if o.optimizer_type == opt.optimizer_type]),
                "avg_improvement": sum(o.improvement for o in self.optimizations if o.optimizer_type == opt.optimizer_type) / len([o for o in self.optimizations if o.optimizer_type == opt.optimizer_type]) if self.optimizations else 0,
                "avg_time": sum(o.execution_time_seconds for o in self.optimizations if o.optimizer_type == opt.optimizer_type) / len([o for o in self.optimizations if o.optimizer_type == opt.optimizer_type]) if self.optimizations else 0,
                "avg_cost": sum(o.total_cost_usd for o in self.optimizations if o.optimizer_type == opt.optimizer_type) / len([o for o in self.optimizations if o.optimizer_type == opt.optimizer_type]) if self.optimizations else 0,
                "avg_lmcalls": sum(o.lm_calls for o in self.optimizations if o.optimizer_type == opt.optimizer_type) / len([o for o in self.optimizations if o.optimizer_type == opt.optimizer_type]) if self.optimizations else 0,
            })
        return dict(groups)
