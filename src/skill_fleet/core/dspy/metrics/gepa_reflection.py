...
def gepa_skill_quality_metric(
    ...
    for field, (_description, weight) in field_checks.items():
    ...
    if hasattr(pred, "outline") and pred.outline:
        found_items = sum(
            1 for item in required_outline_items if item.lower() in str(pred.outline).lower()
        )
        ...
        if found_items < len(required_outline_items):
            missing = [
                item
                for item in required_outline_items
    ...
    if score < 0.7:
        ...
        if "Missing outline" in "\n".join(feedback_parts):
            feedback_parts.append(
                "📌 Try: Request clear section headings (Overview, When to Use, etc.)"
            )

        if "Category mismatch" in "\n".join(feedback_parts):
            feedback_parts.append(
                "📌 Try: Ask the model to confirm the skill's domain/category first"
            )
...
def gepa_semantic_match_metric(
    ...
    has_desc = bool(
        hasattr(pred, "description") and pred.description and len(pred.description) > 20
    )
