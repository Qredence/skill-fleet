...
def generate_smart_options(question_text: str, task_context: str = "") -> tuple[list[str], str | None]:
    ...
    if any(word in question_lower for word in ["language", "python", "typescript", "javascript", "go", "rust"]):
    ...
    elif any(word in question_lower for word in ["project", "codebase", "existing", "new", "legacy"]):
        options = ["New/greenfield project", "Existing/legacy codebase", "Refactoring existing code", "Migration from another tool", "Other"]
        ...
        options = ["Solo developer", "Small team (2-10 people)", "Medium team (10-50 people)", "Large team (50+ people)", "Other"]
        ...
        options = ["Single file or function", "Module or package", "Full project", "Multiple projects", "Other"]

    elif any(word in question_lower for word in ["experience", "skill level", "familiarity"]):
        options = ["Beginner (just learning)", "Intermediate (some experience)", "Advanced (expert)", "Other"]
