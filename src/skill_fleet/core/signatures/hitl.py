...
from ..config.models import ClarifyingQuestion, ValidationCheckItem
...
class GenerateClarifyingQuestions(dspy.Signature):
    ...
    task_description: str = dspy.InputField(
        desc="User's initial task description"
    )
...
class SummarizeUnderstanding(dspy.Signature):
    ...
    intent_analysis: str = dspy.InputField(
        desc="Analyzed intent from Phase 1 parallel analysis"
    )
    ...
    dependencies: list[str] = dspy.InputField(
        desc="List of skill dependencies"
    )
...
class GeneratePreview(dspy.Signature):
    ...
    skill_content: str = dspy.InputField(
        desc="Full generated skill content (SKILL.md)"
    )
    metadata: str = dspy.InputField(
        desc="JSON skill metadata (name, description, capabilities)"
    )
...
class AnalyzeFeedback(dspy.Signature):
    ...
    user_feedback: str = dspy.InputField(
        desc="User's feedback on the preview (free-form text)"
    )
...
class FormatValidationResults(dspy.Signature):
    ...
    skill_content: str = dspy.InputField(
        desc="The skill content that was validated"
    )
...
class GenerateRefinementPlan(dspy.Signature):
    ...
    validation_issues: str = dspy.InputField(
        desc="JSON list of validation issues"
    )
    user_feedback: str = dspy.InputField(
    ...
    # Outputs
    refinement_plan: str = dspy.OutputField(
        desc="Step-by-step plan for refining the skill"
    )
...
class AssessReadiness(dspy.Signature):
    ...
    phase: str = dspy.InputField(
        desc="Current phase: 'understanding', 'generation', 'validation'"
    )
    collected_info: str = dspy.InputField(
        desc="JSON of information collected so far"
    )
    min_requirements: str = dspy.InputField(
        desc="JSON of minimum requirements needed to proceed"
    )
    
    # Outputs
    ready: bool = dspy.OutputField(
        desc="True if ready to proceed to next phase"
    )
    readiness_score: float = dspy.OutputField(
        desc="Readiness score 0-1. >0.8 means ready"
    )
...
class DetermineHITLStrategy(dspy.Signature):
    ...
    reasoning: str = dspy.OutputField(
        desc="Reasoning for this strategy"
    )
